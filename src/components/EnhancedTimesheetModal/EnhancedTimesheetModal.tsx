...
import { LeaveImpactSummary } from "../LeaveImpactSummary";
...
export const EnhancedTimesheetModal: React.FC<EnhancedTimesheetModalProps> = ({
  ...
  // Create period dates array for calculations
  const periodDates = allDates.map((date) => format(date, "yyyy-MM-dd"));

  // FIXED: Remove calendar dependency - no longer disable dates without workday entries
  ...
  const periodTitle = `${userName} - ${format(
    currentDateRange.startDate,
    "MMM dd"
  )} to ${format(currentDateRange.endDate, "MMM dd, yyyy")}`;

  // Calculate summary statistics
  const totalHours = timeSlots.reduce(
    (sum, slot) => sum + (slot.HoursAllocated || 0),
    0
  );
  const totalAvailableHours = Object.values(workdayHours).reduce(
    (sum: number, entry: any) => {
      if (entry === null || entry === undefined) return sum;
      return sum + getHoursFromDayEntry(entry);
    },
    0
  );
  const utilizationPercent =
    totalAvailableHours > 0 ? (totalHours / totalAvailableHours) * 100 : 0;
  const activeGrants = Array.from(
    new Set(timeSlots.map((slot) => slot.GrantID))
  ).length;
                  ...
                  <CardContent>
                    <Box sx={{ display: "flex", alignItems: "center", mb: 2 }}>
                      <Typography variant="body1" sx={{ flexGrow: 1 }}>
                        Time Utilization: {totalHours.toFixed(1)}h /{" "}
                        {totalAvailableHours.toFixed(1)}h
                      </Typography>
                      <Chip
                        label={
                    ...
                    <Typography variant="body2" color="text.secondary">
                      {utilizationPercent > 100
                        ? `Over-allocated by ${(
                            utilizationPercent - 100
                          ).toFixed(1)}%. Consider redistributing hours.`
                        : utilizationPercent > 80
                        ? "Good utilization rate. Time is being used efficiently."
                        : `Under-utilized by ${(
                            100 - utilizationPercent
                          ).toFixed(1)}%. Consider taking on additional work.`}
                    </Typography>
                  </CardContent>
                </Card>
              </Box>
                ...
                >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      console.log(
                        "Export grant breakdown data:",
                        grantFinancialBreakdown
                      );
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    <Card>
                      <CardContent sx={{ textAlign: "center" }}>
                        ...
                        >
                          {formatCurrency(
                            grantFinancialBreakdown.reduce(
                  ...
                  <Grid item xs={12} sm={6} md={4}>
                    <Card>
                      <CardContent sx={{ textAlign: "center" }}>
                        ...
                        >
                          {
                            grantFinancialBreakdown.filter(
                          ...
                          {formatCurrency(
                            grantFinancialBreakdown.length > 0
                              ? grantFinancialBreakdown.reduce(
                                  (sum, item) => sum + item.cost,
                                  0
                                ) / grantFinancialBreakdown.length
                              : 0
                          )}
                        </Typography>
                        <Typography variant="body2" color="text.secondary">
                    ...
                    <CardContent>
                      <Grid container spacing={3}>
                        <Grid item xs={12} md={6}>
                          ...
                          {grantFinancialBreakdown.length > 0 && (
                            <Box>
                              ...
                              >
                                {formatCurrency(
                                  grantFinancialBreakdown[0].cost
                                )}
                                (
                                {grantFinancialBreakdown[0].totalHours.toFixed(
                                  1
                                )}
                                h)
                              </Typography>
                        ...
                        <Grid item xs={12} md={6}>
                          ...
                          <Typography variant="body2" color="text.secondary">
                            {grantFinancialBreakdown.length} grants with an
                            average allocation of{" "}
                            {grantFinancialBreakdown.length > 0
                              ? (
                                  totalHours / grantFinancialBreakdown.length
                                ).toFixed(1)
                              : 0}
                            h per grant
                          </Typography>
