...
export class LocalDynamoDB {
  ...
  private getTableNameFromItem(item: DynamoDBItem): string {
    ...
    if ('AllocationPercent' in item || 'HoursAllocated' in item) return 'timeslots';
  ...
  private applyUpdateExpression(item: DynamoDBItem, expression: string, values: Record<string, any>): DynamoDBItem {
    // Simplified update expression parser
    const updated = { ...item };
    
    // Handle SET operations
    if (expression.includes('SET')) {
      const setClause = expression.split('SET')[1].split('ADD')[0].split('REMOVE')[0].trim();
  ...
  private async validateTimeSlotConstraints(timeSlot: TimeSlot): Promise<void> {
    ...
    const workdayHours = await db.workdayHours.get([timeSlot.PK, `WORKDAY_HOURS#${year}`]);
    const availableHours = workdayHours?.Hours[timeSlot.Date] || DEFAULT_WORKDAY_HOURS;

    // Validate hours constraint
    if (!validateHoursAllocated(allSlots, availableHours)) {
      throw new Error(`Total allocated hours cannot exceed ${availableHours} hours for this day`);
    }
  }
}
